...
path_img_test = "to_test" # folder path

#  net  or weight file path
net_path = "custom-yolov4-detector_best(somewhere 98.62).weights"
...
def find_required_detections(input_image , class_ids , confidences , boxes):
    ...
    for i in range(len(boxes)):
        if i  in indexes:
            ...
            whole_img_name = f"{abc_img[:-4]}_{label}({round(confidences[i]*100 ,2)})_{i}.jpg"
    ...
    for k in required_bb_detections:
        ...
        history_cropped_images[abc_img[:-4]] = cropped_img_dict # global
...
# Path to the directory where images will be uploaded
...
def process_image(filepath):
    ...
    # Perform object detection, segmentation, and calorie prediction
    height,width,channels = img1.shape
    ...
    # Return the processed image and other results
    return result_img

if __name__ == '__main__':
    app.run(debug=True, host='10.12.85.157', ssl_context=('cert.pem', 'key.pem'))
